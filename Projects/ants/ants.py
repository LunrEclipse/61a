...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cur = self.place
        distanceAway = 0
        while(cur.is_hive == False):
            if(len(cur.bees) > 0 and self.min_range <= distanceAway <= self.max_range):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if(self.health - amount <= 0):
            amount += self.damage
            selfDamage = self.health
        copy = list(self.place.bees)
        for b in copy:
            b.reduce_health(amount)
        Ant.reduce_health(self, selfDamage)
...
class HungryAnt(Ant):
    ...
    def __init__(self, health=1, countdown = 0):
        self.chew_countdown = countdown
        super().__init__(health)
    ...
    def action(self, gamestate):
        if self.chew_countdown > 0:
            ...
            eatBee = random_bee(self.place.bees)
            if(eatBee != None):
                eatBee.reduce_health(eatBee.health)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        damageBees = list(self.place.bees)
        for b in damageBees:
