def factors_list(n):
    ...
    [1, 2, 4, 7, 14]
    """
    all_factors = [k for k in range(1,n) if n % k == 0]
    return all_factors
    "*** YOUR CODE HERE ***"
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    location = make_city('loc', lat, lon)
    distA = distance(city_a, location)
    distB = distance(city_b, location)
    if distA < distB:
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        ...
        for i in range(len(leaves)):
            smallerTrees.append(tree(leaves[i]))
        return tree(label(t), smallerTrees)

    smallerTrees = []
    for b in branches(t):
        temp = sprout_leaves(b, leaves)
        smallerTrees.append(temp)
